...
public class KnowledgeDocumentApplicationServiceImpl
    ...
    public KnowledgeDocumentApplicationServiceImpl(KnowledgeDocumentRepository documentRepository,
            PythonAgentClient pythonAgentClient, ObjectMapper objectMapper) {
    ...
    public KnowledgeDocument uploadDocument(String topicId, MultipartFile file) {
        ...
        // 使用充血模型实体构造方法
        KnowledgeDocument doc = new KnowledgeDocument(id, topicId, title, "Admin",
    ...
    public Map<String, Object> initiateIngest(String documentId,
        ...
        IngestDocumentRequest request = new IngestDocumentRequest(
                doc.getFilePath() != null ? doc.getFilePath() : "mock/path.txt", doc.getTopicId(),
                "default", config.chunking().chunkSize(), config.chunking().chunkOverlap(),
                config.chunking().separators(), config.indexing().embeddingModel(),
                config.indexing().vectorStore(), config.retrieval().topK(),
                config.retrieval().scoreThreshold(), config.retrieval().enableHybridSearch(),
                config.retrieval().alphaWeight(), config.generation().generationModel(),
                config.generation().temperature(), config.generation().maxTokens(),
                config.generation().systemPrompt(), config.evaluation().enableEvaluation(),
    ...
    private KnowledgeConfig convertToConfig(Map<String, Object> payload) {
        try {
            ...
            return new KnowledgeConfig(
                    new KnowledgeConfig.ChunkingConfig((Integer) payload.get("chunkSize"),
                            (Integer) payload.get("chunkOverlap"),
                            (List<String>) payload.get("separators")),
                    new KnowledgeConfig.IndexingConfig((String) payload.get("embeddingModel"),
                            (String) payload.get("vectorStore")),
                    new KnowledgeConfig.RetrievalConfig((Integer) payload.get("topK"),
                            payload.get("scoreThreshold") != null
                                    ? Double.valueOf(payload.get("scoreThreshold").toString())
                                    : null,
                            (Boolean) payload.get("enableHybridSearch"),
                            payload.get("alphaWeight") != null
                                    ? Double.valueOf(payload.get("alphaWeight").toString())
                                    : null),
                    new KnowledgeConfig.GenerationConfig((String) payload.get("generationModel"),
                            payload.get("temperature") != null
                                    ? Double.valueOf(payload.get("temperature").toString())
                                    : null,
                            (Integer) payload.get("maxTokens"),
                            (String) payload.get("systemPrompt")),
                    new KnowledgeConfig.EvaluationConfig((Boolean) payload.get("enableEvaluation"),
