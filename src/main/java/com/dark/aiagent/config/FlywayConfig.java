...
/**
 * Flyway 延迟初始化配置 确保在 Nacos 配置完全加载且应用就绪后执行数据库迁移
 */
@Slf4j
@Configuration
...
public class FlywayConfig {
    ...
    public Flyway flyway() {
        return Flyway.configure().dataSource(url, user, password)
                ...
                .baselineOnMigrate(true).validateOnMigrate(false).connectRetries(20)
    ...
    public void onApplicationReady(ApplicationReadyEvent event) {
        ...
        try {
            ...
            // 方式2: 如果sql脚本变化需repair 从上下文中获取 Bean 确保单例执行
            Flyway flyway = event.getApplicationContext().getBean(Flyway.class);
            log.info("【Flyway】执行 repair 以同步校验和并修复失败状态...");
            flyway.repair();
            flyway.migrate();
